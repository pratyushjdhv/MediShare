...
class pharmacy(db.Model,UserMixin):
    pharmaid = db.Column(db.Integer,autoincrement=True)
    license_no = db.Column(db.String(12),primary_key=True)
    pharma_name=db.Column(db.String(65))
    pincode = db.Column(db.Integer, nullable=False)
    email=db.Column(db.String(320),nullable=False)
    password=db.Column(db.String(length=65),nullable=False)
    ...
    @password.setter
    def set_pass(self,txt_password):
        self.password=bcrypt.generate_password_hash(txt_password).decode("utf-8")
